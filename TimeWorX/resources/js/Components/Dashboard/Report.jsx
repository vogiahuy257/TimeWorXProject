...
export default function Report({ auth }) {
    ...
    return (
        <section id='report' className='rounded-lg overflow-y-auto scrollbar-hide'>
            <Head title="Report" />
            <div className="main w-full p-2 flex flex-col justify-center items-center">
                {projects.length > 0 ? (
                    <div className='w-full h-full'>
                        {/* Danh sách Project */}
                        <div className='w-full h-1/4 flex flex-col lg:flex-row'>
                            
                            <div className={`report-content report-project m-2 p-4 rounded-md shadow-md lg:w-3/5`}>
                                <h2 className="text-header">Project</h2>
                                <div className="project-box flex flex-wrap gap-2 p-2 mt-1 w-full rounded max-h-[220px] overflow-y-auto scrollbar-hide">
                                {projects.map((project) => (
                                    <button 
                                        key={project.project_id}
                                        onClick={() => handleProjectClick(project)}
                                        className={`py-2 rounded-md  ${selectProject.project_id == project.project_id ? "active" : ""}`}
                                    >
                            ...
                            <div className={`h-52 report-content detail-box m-2 p-4 rounded-md relative lg:w-2/5 lg:h-auto`}>
